...
[Route("companies")]
public class CompanyController(CompanyService companyService, CityService cityService, FilterService filterService, VacancyService vacancyService, LanguageService languageService)
    : Controller
{
    [HttpGet("")]
    public async Task<IActionResult> Index(string? search, int page = 1, string? sort = null)
    {
        ...
        var (items, totalCount) = await companyService.GetPagedListAsync(search, page, pageSize, sort);
    ...
    {
        filterParams.Lang = languageService.GetCurrentLanguage();
